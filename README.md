...
- `draft` 必须显式填写。只有 `draft: false` 且发布日期不晚于北京时间当天的文章会进入页面、搜索和 RSS。定时文章在发布日期的北京时间零点后自动出现，无需重新发布。
- 日期必须加引号。修改文章时可添加 `updatedAt: "2026-09-16"`，不能早于发布日期。
- `topic` 可使用任意小写字母、数字和 ASCII hyphen 组成的标识，例如 `machine-learning`。专题从已发布文章自动汇总，按最近发文顺序显示；草稿和定时未发布文章不会提前公开专题。
- `cover` 可省略（纯文字卡片）、填写内容仓库的图片路径（如 `/media/covers/docker.webp`）或 HTTPS 图片地址。原有 `layers`、`branches`、`brackets`、`search` 图形仍可使用。
...
本地配图使用私有浏览器缓存，每次复用前仍向源站确认：图片未变时返回无正文的 `304`，替换后返回新图片，失去所有已发布引用后返回 `404`。不要用 Cloudflare 规则强制覆盖 `/media/` 的缓存头，否则撤回和回滚可能延迟生效。部署与访问速度的检查结果和后续建议见 [PERFORMANCE.md](PERFORMANCE.md)。
...
非草稿的定时文章会提前编译并写入内部搜索索引；查询使用北京时间的发布日期过滤，公开结果再次按当前已发布文章筛选。这样文章到期即可搜索，草稿始终不进入索引。应用部署时也会重建索引以应用当前搜索配置。

文章末尾的「版本历史」展示 Git 提交时间（北京时间）、提交说明和短版本号。发布时从该内容版本的 Git 历史生成记录，支持 Git 能识别的文件改名；回滚时记录随文章一起回滚。只展示相同文章 `id`、`draft: false` 且发布日期已到的记录，不输出提交邮箱、历史正文或仓库地址。历史按主分支演进顺序排列，每篇最多检查最近 100 次相关提交；未提交的本地修改不冒充已提交版本。提交说明将公开展示，建议用它概括本次文章修改。

历史元数据保存在内容版本目录的 `.article-history.json`，由发布脚本自动生成。升级应用时也会为现有内容补齐，无需修改内容仓库的 workflow 或增加 Secret。只有维护容器在生成时临时只读挂载 Git 对象库，网页服务不需要 Git 或访问 GitHub。本地开发会随内容提交自动更新；没有 Git 历史的文章不显示历史入口。
...
当前和上一版本目录不会被失败清理或历史清理删除；新应用对当前内容校验失败时也保留原目录。搜索交换后保留旧索引直到检查通过。失败或中断恢复会切回旧内容，并从旧目录重建搜索索引；恢复未完成时保留 journal，后续操作再次尝试，不会重复交换状态未知的索引。
