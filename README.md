...
读者通过 GitHub 登录后，可以评论、回复、点赞、收藏和管理个人资料。管理员可以管理用户、审核评论并查看操作记录。文章使用 MDX 编写，通过 GitHub Actions 构建和发布，生产服务由 Docker Compose 管理，并通过 Cloudflare Tunnel 接入。
...
在 `content/posts/` 新建使用小写字母、数字和 ASCII hyphen 命名的 `.mdx` 文件：
...
提交源码、MDX、依赖锁文件、数据库迁移、测试、容器配置和维护脚本。测试是 CI 检查的一部分，迁移是初始化及升级数据库的必要文件。
...
1. 将部署文件放入服务器部署用户的 `~/apps/world-blog`。提供的 Docker 安装脚本适用于 Ubuntu 22.04，已有 Docker 时仅检查版本。
   ...
   bash scripts/ops/init-env.sh https://www.world9566.online ghcr.io/你的账号/你的仓库
   ```
